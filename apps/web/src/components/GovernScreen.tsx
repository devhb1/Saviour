...
export function GovernScreen({
  ...
  async function dispute(address: string) {
    ...
    try {
      ...
      const json = (await res.json()) as { error?: string; dispute?: { status: string } };
      if (!res.ok) throw new Error(json.error ?? `HTTP ${res.status}`);
      setNote(`Disputed → ${json.dispute?.status ?? "WATCH"}`);
      await load();
    } catch (e) {
  ...
  async function revoke(address: string) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json.error ?? `HTTP ${res.status}`);
      setNote("Revoked ENS name (registry append-only)");
      await load();
    } catch (e) {
  ...
  return (
    <section className="rise">
      ...
      {note ? (
        <p style={{ color: "var(--signal)", fontSize: 14 }}>{note}</p>
      ) : null}
